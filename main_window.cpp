...
void main_window::setup_format_menu() {
    ...
    const auto *color_action = format_menu->addAction("Text Color...");
    connect(color_action, &QAction::triggered, this, [this] {
        const QColor color = QColorDialog::getColor(Qt::black, this);
...
void main_window::update_status_bar() const {
    ...
    static const QRegularExpression wordRegex("\\s+");

    const QStringList words = text.split(wordRegex, Qt::SkipEmptyParts);
    const long long wordCount = words.count();
...
void main_window::show_context_menu(const QPoint &pos) {
    ...
    if (!word.isEmpty() && !checker.isCorrect(word.toStdString())) {
        std::vector<std::string> options = checker.suggestions(word.toStdString());
